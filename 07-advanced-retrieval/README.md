...
| 02 | [BM25](02-bm25/) | The Okapi BM25 scoring function implemented by hand, then via rank_bm25; the role of k1 and b. | `not started` |
| 03 | [Sparse vs Dense Retrieval](03-sparse-vs-dense/) | Where lexical matching wins (rare terms, IDs, exact phrases) and where dense wins (paraphrase, intent). | `not started` |
| 04 | [Hybrid Search](04-hybrid-search/) | Combining sparse and dense results; score normalisation and weighted fusion. | `not started` |
| 05 | [Reciprocal Rank Fusion](05-reciprocal-rank-fusion/) | Rank-based fusion that avoids score normalisation entirely; implementing RRF from the formula. | `not started` |
...
| 10 | [Cross-Encoder Reranking](10-cross-encoder-reranking/) | Joint query-document encoding: why it is more accurate than bi-encoders and why it cannot scale to the whole corpus. | `not started` |
